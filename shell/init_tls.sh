...
# 恢复配置
resetNginxConfig(){
    `cp -Rrf /tmp/panhuanghe/nginx/nginx.conf /etc/nginx/nginx.conf`
    rm -rf /etc/nginx/conf.d/5NX2O9XQKP.conf
    echoColor green "\n恢复配置完毕"
}
# 备份
bakConfig(){
    mkdir -p /tmp/panhuanghe/nginx
    `cp -Rrf /etc/nginx/nginx.conf /tmp/panhuanghe/nginx/nginx.conf`
}
# 安装证书
installTLS(){
    ...
    ~/.acme.sh/acme.sh --installcert -d ${domain} --fullchainpath /tmp/panhuanghe/nginx/${domain}.crt --keypath /tmp/panhuanghe/nginx/${domain}.key --ecc >/dev/null
    if [[ -z `cat /tmp/panhuanghe/nginx/${domain}.key` ]]
    then
        echoColor red "证书key生成失败，请重新运行"
        resetNginxConfig
        exit
    elif [[ -z `cat /tmp/panhuanghe/nginx/${domain}.crt` ]]
    then
    ...
    echoColor green "证书目录/tmp/panhuanghe/nginx"
    ls /tmp/panhuanghe/nginx
...
init(){
    ...
    echoColor green "   12.Github[https://github.com/panhuanghe]"
    echoColor red "=============================="
